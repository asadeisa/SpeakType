...
- `pi` is slow cold (~30–120s). Don't busy-wait. If `Monitor` is available, pace with a single
  tick, then read once. If it is unavailable, continue useful work and read the pane later.
...
- The **Bash tool's `fork` is broken** here (`dofork … Resource temporarily unavailable`, exit 254).
  Don't pace with bash `sleep`/`until`. Use `Monitor` if available, or just re-read the pane after
  other work.
- Standalone `sleep` / `Start-Sleep` may be blocked by the harness. Use `Monitor` when available;
  otherwise continue useful work and re-read the pane later.
...
6. Monitor one tick if available, or re-read later → terminal_read → relay the reply
